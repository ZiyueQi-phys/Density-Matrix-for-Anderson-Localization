...
def custom_func_single(x, a, b, c, d, f, xi):
    return a*np.exp(-b*x*1) + d*np.exp(-f*x)*np.cos(pi*x)
...
y1 = a1*np.exp(-x1*b1) + + d1*np.exp(-f1*x1)*np.cos(pi*x1)
...
plt.yticks([1E-5,2E-5,3E-5,4E-05])

plt.savefig('C_list_single_V-0.9_W1.svg')

...
def get_TMM(M,g,W1,W2,EF):
    TM = np.zeros((M*2, M*2),  dtype=np.complex128)

    for ii in range(M):
        TM[ii, ii] = (EF-random.uniform(W1,W2))/g
    for ii in range(M):
        if(M==2 and ii==1):
            continue
        TM[ii, (ii+1)%M] = -1
...
def get_localization_length(M,g,W,EF,NL):
    ...
    TM = np.eye((2*M),dtype=complex)

    R_log_list = np.zeros((2*M),dtype=float)

    R_temp_list = np.zeros((2*M),dtype=float)
    R_accumulate = []
    R_average_list = [0]
...
def main():
    ...
    for nn in range(1):
        ...
        for ii in range(num_W):
            ...
            print("W:",W)
            for ss in range(num_iter):
                print("num_iter:",ss)
                leng_temp = get_localization_length(M,g,W,EF,NL)
                leng = leng + leng_temp/num_iter
            #print("W_list:",W_list)
...
W_list = [1.4 + ii*0.2 for ii in range(15)]

for ii in range(len(W_list)):
    
    folder_name = 'W'+"{:.2f}".format(W_list[ii])
    #folder_name = str(W_list[ii])
    os.makedirs(folder_name, exist_ok=True)
    
    
    modified_code = content.replace("?????", str(W_list[ii]) )
    
    
    file_path = os.path.join(folder_name, "Transfer_Matrix_2D_length.py")
    with open(file_path, 'w') as f:
        f.write(modified_code)
    f.close()


print("Code files Sucessfully constructed !!!!!")

...
for ii in range(len(M_list)):
    ...
    plt.plot(1*np.array(W_list) ,np.array(Length_list),'-',color=color_list[ii],linewidth = 1.5)

    plt.errorbar(1*np.array(W_list), np.array(Length_list), 
                 yerr=np.array(Err_list)/sqrt(5),  # y方向的误差\
                 fmt='-',       # 线条和标记样式
                 color=color_list[ii], 
                 linewidth=1.5, 
                 capsize=3,      # 误差线端帽长度

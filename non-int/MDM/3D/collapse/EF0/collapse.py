...
for ii in range(len(M_list)):
    ...
    for jj in range(len(W_list)):
        ...
        F_list.append( np.log((W-Wc)**(1.58)*M)  )

        if(mean_value/M<0.572):
            print(111)
            print(np.log((W-Wc)**(1.58)*M))
            F_total_list_1.append( np.log((W-Wc)**(1.58)*M) )
            Length_total_list_1.append(mean_value/M)
        else:
            print(222)
            F_total_list_2.append( np.log((W-Wc)**(1.58)*M) )
